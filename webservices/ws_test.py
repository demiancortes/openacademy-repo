...
sessions = call(
    "openacademy.session", "search_read", [], ["name", "seats", "taken_seats"]
)
for session in sessions:
    print(
        "Session %s (%s seats) Taken Seats %d"
        % (session["name"], session["seats"], round(session["taken_seats"], 0))
    )

users = call(
    "res.users", "search_read", [("id", "=", uid)], ["name", "company_id", "password"]
)
